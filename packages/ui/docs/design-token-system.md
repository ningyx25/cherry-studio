# HuaTuo Studio Shadcn Variable System
...
This document defines the current variable system for HuaTuo Studio. It is intentionally focused on the Shadcn
...
| unprefixed product semantics | Consumer-backed HuaTuo Studio extensions | Stable public API when Shadcn has no matching role |
...
separate Shadcn and HuaTuo Studio ownership inventories over the values already shipped by HuaTuo Studio:
...
3. unprefixed HuaTuo Studio product semantics in the same public namespace as the Shadcn contract;
...
`product.css` is the authored HuaTuo Studio product layer. Every entry is stable public API. Historical renderer
...
### 3.4 HuaTuo Studio product semantic layer
...
All product variables are stable, consumer-backed HuaTuo Studio semantics not covered by Shadcn.
...
### 4.3 HuaTuo Studio product color extensions
