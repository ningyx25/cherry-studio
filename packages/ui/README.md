...
HuaTuo Studio UI component library for React applications.

## ✨ Features

- 🎨 **Design System**: HuaTuo Studio primitive palettes, product semantics, and Shadcn-compatible theme mappings
...
Use the full HuaTuo Studio design system so Tailwind theme tokens resolve to HuaTuo Studio values.
...
- ✅ Colors resolve to HuaTuo Studio design values
...
<Button className="bg-primary text-red-500 p-4 rounded-lg">
  ...
  {/* text-red-500 -> HuaTuo Studio red-500 */}
...
@theme inline {
  --color-primary: var(--cs-brand-500); /* Adopt a HuaTuo Studio foundation value */
...
- ✅ Gives access to HuaTuo Studio foundation values (`var(--cs-brand-500)`, `var(--cs-red-500)`)
- ✅ Lets you choose what to adopt and what to keep
- ✅ Works when you already own the semantic contract and only need selected HuaTuo Studio foundations
- ⚠️ Does not expose the complete Shadcn or HuaTuo Studio product contract
...
HuaTuo Studio product variables extend the same unprefixed public namespace. Use the
...
2. Approved HuaTuo Studio product semantics are also unprefixed, such as `--success` and `--background-subtle`
...
Use the Shadcn CLI to scaffold or update component source and dependency metadata only. HuaTuo Studio's authored
...
- Review any CSS proposed by `shadcn add` and place it according to ownership: official semantics in `shadcn.css`,
  HuaTuo Studio product semantics in `product.css` and `theme-contract.ts`, and component-local styles with the
  component.
...
When HuaTuo Studio consumes the package source, the app's UI-contract generator treats those markers as structural
