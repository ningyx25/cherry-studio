...
description: 收集、脱敏、预览并提交 HuaTuo Studio BUG、UI/UX 或功能反馈，默认提交到飞书。可在用户同意后调用内置诊断工具整理环境、错误日志、截图和用户导出的 trace，自动提交飞书表单或生成匿名上传 ZIP；也可安全解析反馈 ZIP 为表单字段。用户说“提交问题”“提交反馈”“上报 bug”“收集/上传错误信息”“整理日志/trace”“生成反馈包”，或描述 HuaTuo Studio 问题并希望记录时触发。只有明确要求 GitHub Issue 时才改用 issue-reporter。
---

# HuaTuo Studio Feedback
...
优先使用 `info` 返回的真实应用版本，不硬编码版本号。删除绝对路径和主机名，只保留平台、系统版本、架构、内存概况、HuaTuo Studio/Electron/Node 版本，以及与问题有关的脱敏配置。
