...
const NAVIGATE_TOOL: Tool = {
  name: 'navigate',
  description:
    'Create a clickable entry for a route returned by product_info. Use this in the same turn whenever answering where to find, open, configure, or use a HuaTuo Studio page or feature; written UI steps are not a substitute.',
  inputSchema: {
    type: 'object',
    properties: {
      path: {
...
const DIAGNOSE_TOOL: Tool = {
  name: 'diagnose',
  description:
    'Read HuaTuo Studio runtime state for troubleshooting. Use this to inspect app info, provider config, connectivity, logs, and MCP server status.',
  inputSchema: {
    type: 'object',
    properties: {
      action: {
...
const PRODUCT_INFO_TOOL: Tool = {
  name: 'product_info',
  description:
    'Read current HuaTuo Studio product facts from the installed package manifest. Request only the relevant section to keep context small.',
  inputSchema: {
    type: 'object',
    properties: {
      source: {
...
const APPLY_SETTING_TOOL: Tool = {
  name: 'apply_setting',
  description: `Apply a low-risk HuaTuo Studio setting change directly. Only the whitelist below is supported; destructive operations are never exposed here.
  ...
  private async createAgent(args: Record<string, string | undefined>) {
    ...
    try {
      ...
      if (isDataApiError(error) && error.code === DataApiErrorCode.NOT_FOUND) {
        throw new McpError(ErrorCode.InvalidParams, `Model is not configured in HuaTuo Studio: ${parsedModel.data}`)
      }
      throw error
    }
