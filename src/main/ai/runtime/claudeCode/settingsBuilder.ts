/**
 * Builds ClaudeCodeSettings from HuaTuo Studio's agent session configuration.
 *
 * Maps HuaTuo Studio's internal data model (agent sessions, providers, MCP servers,
...
/**
 * Build session-level ClaudeCodeSettings from HuaTuo Studio's agent session.
 */
    ...
    return {
      hookSpecificOutput: {
        ...
        permissionDecisionReason:
          'Headless channel or scheduled turns cannot mutate agent configuration. Ask the user to make this change in HuaTuo Studio.'
      }
    }
  }
    ...
    if (interactionState.currentTurn === 'headless' || interactionState.userResponse === 'unavailable') {
      return {
        hookSpecificOutput: {
          ...
          permissionDecision: 'deny',
          permissionDecisionReason:
            'Headless channel or scheduled turns cannot submit HuaTuo Studio feedback. Keep the local feedback draft for an interactive user to review and submit.'
        }
      }
    }
    return {
      hookSpecificOutput: {
        hookEventName: 'PreToolUse',
        permissionDecision: 'ask',
        permissionDecisionReason: 'Submitting HuaTuo Studio feedback to Feishu requires live per-call user approval.'
      }
    }
  }
