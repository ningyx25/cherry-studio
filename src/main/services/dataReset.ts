...
export function runDataReset(): void {
  try {
    ...
    try {
      ...
      showDataResetError(
        'Data Reset Failed',
        'HuaTuo Studio could not record the data reset state ' +
          `in ${markerPath()}.\n\n` +
          'Starting now could erase data you create later, so the app will quit instead.\n\n' +
          'Please check disk space and file permissions, then start HuaTuo Studio again.'
      )
      return
    }
    ...
    try {
      ...
      showDataResetError(
        ...
        'HuaTuo Studio erased its data but could not record the reset as finished ' +
          `in ${markerPath()}.\n\n` +
    ...
    showDataResetError(
      'Data Reset Failed',
      'HuaTuo Studio could not safely complete a pending data reset. ' +
...
function showPathMismatchWarning(): void {
  dialog.showErrorBox(
    ...
    'HuaTuo Studio did not run Data Reset because the data location changed after confirmation.\n\n' +
...
function showIncompleteResetWarning(): void {
  dialog.showErrorBox(
    'Data Reset Incomplete',
    'HuaTuo Studio could not remove some of its data during the data reset.\n\n' +
