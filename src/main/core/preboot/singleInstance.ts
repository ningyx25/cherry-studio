...
/**
 * Require this process to be the primary HuaTuo Studio instance.
 *
 * Claims Electron's single-instance lock via `app.requestSingleInstanceLock()`.
 * If another HuaTuo Studio process already holds the lock, this function
...
export function requireSingleInstance(): void {
  ...
  logger.info('Another HuaTuo Studio instance already holds the single-instance lock; exiting')
  application.quit()
  process.exit(0)
}
