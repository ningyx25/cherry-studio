...
describe('getNormalizedExecutablePath', () => {
  it('macOS: returns app.getPath("exe") verbatim', async () => {
    ...
    stubElectron({ exePath: '/Applications/HuaTuo Studio.app/Contents/MacOS/HuaTuo Studio' })
    stubBootConfig()
    stubFs()
    const { getNormalizedExecutablePath } = await loadModule()
    expect(getNormalizedExecutablePath()).toBe('/Applications/HuaTuo Studio.app/Contents/MacOS/HuaTuo Studio')
  })
  ...
  it('Windows non-portable: returns app.getPath("exe") verbatim', async () => {
    ...
    stubElectron({ exePath: 'C:\\Program Files\\HuaTuo Studio\\HuaTuo Studio.exe' })
    stubBootConfig()
    stubFs()
    const { getNormalizedExecutablePath } = await loadModule()
    expect(getNormalizedExecutablePath()).toBe('C:\\Program Files\\HuaTuo Studio\\HuaTuo Studio.exe')
  })
  ...
  it('Windows portable: returns PORTABLE_EXECUTABLE_DIR/cherry-studio-portable.exe', async () => {
    ...
    stubElectron({ exePath: 'D:\\PortableApps\\CherryStudio\\HuaTuo Studio.exe' })
    stubBootConfig()
    stubFs()
    const { getNormalizedExecutablePath } = await loadModule()
