...
// SubWindow is used only as a fixture slot here — we overwrite its entry with a minimal
// fixture per test, so these tests never couple to the real registry config.
const fixtureKey = WindowType.SubWindow // reuse the enum value; we overwrite the entry below
